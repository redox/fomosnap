...
ScrollCapturePanel::ScrollCapturePanel(MonitorInfo monitor, QWidget *parent)
...
void ScrollCapturePanel::applyInputRegion() {
  ...
  if (phase_ != Phase::Capturing && phase_ != Phase::Selected) {
    ...
    macwindow::hideFromCapture(window);
    return;
  }
...
void ScrollCapturePanel::updateKeyboardZone(const QPoint &point) {
  // Held only on our chrome: over the hole the page has to take the wheel.
  setKeyboardGrab(!region_.contains(point));
}

void ScrollCapturePanel::setKeyboardGrab(bool grab) {
  if (keyboardGrabbed_ == grab)
    return;
  keyboardGrabbed_ = grab;
  macwindow::setKeyboardGrab(surfaceWindow(), grab);
}
...
void ScrollCapturePanel::startCapture(Mode mode, stitch::Axis axis) {
  ...
  worker->debugDir = qEnvironmentVariable("FOMOSNAP_SCROLL_DEBUG_DIR");
  ...
  repaint();
  // The page has to take the wheel for the whole capture. Finish and cancel
  // run off the on-screen buttons (clicks, governed by the input mask).
  ...
  const QPoint scrollAt = autoScrollParkPoint();
  ...
  QTimer::singleShot(
      ...
      [this, scrollAt] {
        if (phase_ != Phase::Capturing)
          return;
        QString spawnError;
        if (!spawnScrollInjector(injectorStop_, handshake_, scrollAt,
                                 axis_, spawnError)) {
          // Fall back to manual capture on the same region and axis.
          qInfo().noquote()
              << QStringLiteral("scroll: injector unavailable (%1)").arg(spawnError);
          // Missing permission is the one failure the user can act on, so ask
          // for it -- once, and only when auto-scroll was actually requested.
          // Manual scrolling needs nothing and keeps working meanwhile.
          if (!scrollInjectionPermitted() && !askedForInjectionPermission_) {
            askedForInjectionPermission_ = true;
            requestScrollInjectionPermission();
          }
          mode_ = Mode::Manual;
          setStatus(QStringLiteral("Auto-scroll needs Accessibility · scroll "
...
QPoint ScrollCapturePanel::autoScrollParkPoint() const {
  ...
  // once the page has scrolled it under a fixed park point. Global, in
  ...
  return monitor_.geometry.topLeft() +
         QPoint(qRound(region_.x() + region_.width() - insetX),
                qRound(region_.y() + region_.height() - insetY));
}
...
void ScrollCapturePanel::continueCapture() {
  ...
  // capture of the same page. The injector addresses the same park point
  // inside the frame; the pointer itself stays where the user left it.
  if (phase_ != Phase::Capturing || !worker_ || mode_ != Mode::Auto)
    return;
  autoStalled_ = false;
  stopRequested_ = false;
  // Continue was clicked on our chrome, so drop the keyboard before the
  // injector addresses the page again.
  setKeyboardGrab(false);
