 ...
 *  into one tall image with the pure stitcher. Ties together the input-mask
 *  hole, live ScreenCaptureKit grabs, and stitch.hpp.
...
/// manages the input-mask hole and keyboard while it is up, and
/// hands back with stitched(), dismissed() or tabRequested().
class ScrollCapturePanel final : public QWidget {
  Q_OBJECT
public:
  /// The panel releases the overlay's keyboard grab and punches an input hole
  /// over the capture region while it is up, so the page underneath stays
  /// live, and restores both on destruction.
  ScrollCapturePanel(MonitorInfo monitor, QWidget *parent);
  ~ScrollCapturePanel() override;
  /// Restores the overlay's mask and keyboard. Safe to call more than once;
  /// the destructor does. The editor calls this before deleteLater so the
  /// hole is gone in the same event that dismissed the panel.
  void detachSurface();
...
private:
  ...
  /// Takes or releases the overlay's keyboard. Released while a region exists
  /// so the page underneath can take the wheel.
  ...
  /// Where auto-scroll wheel events are addressed (global logical points):
  /// the region's bottom-right corner, inset 10% so it stays inside it.
  [[nodiscard]] QPoint autoScrollParkPoint() const;
  ...
  /// The Accessibility prompt is raised at most once per panel: it cannot be
  /// granted in-process, so asking again in the same session is only noise.
  bool askedForInjectionPermission_ = false;
  ...
  /// Whether the overlay currently holds the keyboard: only while the pointer
  /// is on our chrome, never over the live page.
